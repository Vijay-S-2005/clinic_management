...
export default function Medicinelist() {
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      try {
        ...
        const uniqueGroups = [
          ...new Set(response.data.data.map((medicine) => medicine.groupName)),
        ];
        setGroups(uniqueGroups);

        const uniqueForms = [
          ...new Set(response.data.data.map((medicine) => medicine.dosageForm)),
        ];
  ...
  const filteredMedicines = medicines.filter(
    (medicine) =>
      ...
      (selectedExpiry === "" ||
        new Date(medicine.expiryDate) <= new Date(selectedExpiry))
  );
      ...
      <div className="flex-1 p-6">
        ...
        <div className="mb-4">
          <h2 className="text-2xl font-bold">
            Medicine Inventory ({totalResults})
          </h2>
          <p className="mb-4 text-gray-600">
            Details of medicines available in stock.
          </p>
        </div>
            ...
            <tbody>
              {currentMedicines.map((medicine) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <Link
                      href={`/medicinedetails/${medicine.medicine_id_auto}`}
                    >
